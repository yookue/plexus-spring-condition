...
import cn.unikue.commonplexus.springcondition.annotation.ConditionalOnLinux;


/**
 * Condition being active when matching linuxOS
 *
 * @author David Hsing
 */
...
public class OnLinuxCondition extends SpringBootCondition {
    private static final Class<? extends Annotation> annotation = ConditionalOnLinux.class;
    ...
    public ConditionOutcome getMatchOutcome(@Nonnull ConditionContext context, @Nonnull AnnotatedTypeMetadata metadata) {
        ...
        return SystemUtils.IS_OS_LINUX ? ConditionOutcome.match(builder.available(equation)) : ConditionOutcome.noMatch(builder.notAvailable(equation));
    }
}
